...
class CelebA_HQ_loader(data.Dataset):
    ...
    def __init__(self,
        ...
        self.df = pd.read_csv(self.landmark_root, index_col=0)
    ...
    def __getitem__(self, index):
        ...
        lr_img = sr_img.resize((int(128 / scale), int(128 / scale))).resize((112, 112), Image.BICUBIC)
        ...
        # ****************  Parsing Map     **********************
        parsing_label = Image.open(os.path.join(self.parsing_root, img_name.split('.')[0].zfill(5) + '.png'))
        parsing_label = parsing_label.resize((128, 128)).rotate((random_angle)).crop((nw, nh, nw + 112, nh + 112))
        parsing_label = np.array(parsing_label)
        label_mask = np.zeros((parsing_label.shape[0], parsing_label.shape[1]), dtype=np.int16)
        for i in range(parsing_label.min(), parsing_label.max() + 1):
            label_mask[np.where(np.all(parsing_label == i, axis=-1))] = i
        
        # *****************     Landmark    *********************
        landmark = self.df.loc[img_name].values.reshape(-1, 2)
        landmark = np.dot(landmark - 64, self.rotate_matrix(random_angle)) + 64
        landmark[:, 0] = landmark[:, 0] - int(nw)
        landmark[:, 1] = landmark[:, 1] - int(nh)
        heatmap = self.generate_hm(height=112, width=112, landmark=landmark, s=2.0)
        
        if self.is_transform:
            lr_img, sr_img, label_mask, heatmap = self.transform(lr_img, sr_img, label_mask, heatmap)
        
        batch = {}
        batch['lr_img'] = lr_img
        batch['hr_img'] = sr_img
        batch['parsing_label'] = label_mask
